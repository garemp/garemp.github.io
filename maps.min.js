...
edges=s.graph.edges();if(drawcvco=!drawcvco)for(j=0;j<edges.length;j++)edges[j].color=edges[j].defaultcolor;else for(j=0;j<edges.length;j++)edges[j].color="#000";s.refresh()});$("a").mouseover(function(b){h=parseInt($(this).attr("height"));$("#"+$(this).attr("id")+"-pane").css({position:"absolute",top:b.pageY-20*h,left:b.pageX+10});$("#"+$(this).attr("id")+"-pane").show()});$("a").mouseout(function(){$("#"+$(this).attr("id")+"-pane").hide()})});
