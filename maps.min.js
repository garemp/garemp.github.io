...
$(this).attr("src",d)):($("#comp-name").text(e),$(this).attr("src",a));$(this).attr("width","600px");300<$(this).height()?($(this).attr("height","300px"),$(this).removeAttr("width"),$(this).css({top:"0px"}),$(this).css({left:(600-$(this).width())/2+"px"})):($(this).css({left:"0px"}),$(this).css({top:(300-$(this).height())/2+"px"}))});$("#comp-close").click(function(){$("#comp-div").empty().hide()})})}
...
b.attributes[i]||"equal-area"==b.attributes[i]||"equidistant"==b.attributes[i]||"compromise"==b.attributes[i])e=e?e+(", "+b.attributes[i]):e+b.attributes[i];if("cylindrical"==b.attributes[i]||"pseudocylindrical"==b.attributes[i]||"conic"==b.attributes[i]||"pseudoconic"==b.attributes[i]||"azimuthal"==b.attributes[i]||"lenticular"==b.attributes[i])d+=b.attributes[i]}b.img&&$(c).append("<img class='paneimg' src=\""+b.img+'" width=100%></img>');$(".paneimg").waitForImages(!0).done(function(){20==$(this).height()&&
$(this).attr("title","No preview image available")});d&&$(c).append("<p>Type: "+d+"</p>");e&&$(c).append("<p>Properties: "+e+"</p>");$(c).show();if(a){nextptr=0;neighbor=b.neighbors;for(j=0;j<edges.length;j++)edges[j].color="#000";visnb=[];visedge=[];b.edges.forEach(function(a,c){0!=c&&(edges[a].color=edges[a].defaultcolor,vis=nodes[b.neighbors[c]].cat1viz&&nodes[b.neighbors[c]].cat2viz&&nodes[b.neighbors[c]].yearviz)&&(visnb.push(b.neighbors[c]),visedge.push(b.edges[c]))});ptrlen=Object.values(visedge).length;
0!=ptrlen&&($(c).append('<hr><div id="show-rel" style="text-align:center"><button id="show-btn">show relations ('+ptrlen+")</button></div>"),$(c).append('<div id="rel-area"</div>'),s.refresh(),$("#show-btn").click(function(){1<ptrlen?$("#show-btn").text("next relations"):$("#show-btn").hide();for(j=0;j<edges.length;j++)edges[j].color="#000";nextptr>=ptrlen&&(nextptr=0);edges[visedge[nextptr]].color=edges[visedge[nextptr]].defaultcolor;s.refresh();$("#rel-area").empty();$("#rel-area").append("<p><< see <b>"+
nodes[visnb[nextptr]].label+" projection</b> ("+(nextptr+1)+"/"+ptrlen+') in left side panel&nbsp;<button id="comp-btn">compare!</button></div></p>');1<ptrlen&&$("#rel-area").append('<div style="text-align:center"><button id="prev-btn">prev relations</button></div>');compareMap(b,nodes[visnb[nextptr]]);$("#prev-btn").click(function(){nextptr--;nextptr--;0>nextptr&&(nextptr=ptrlen-1);$("#show-btn").click();$("#comp-div").empty().hide()});updatePane("#minor-pane",nodes[visnb[nextptr]],!1);$("#minor-pane").append("<hr>");
