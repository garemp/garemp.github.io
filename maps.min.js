...
$jscomp.getGlobal=function(c){c=["object"==typeof window&&window,"object"==typeof self&&self,"object"==typeof global&&global,c];for(var b=0;b<c.length;++b){var a=c[b];if(a&&a.Math==Math)return a}return globalThis};$jscomp.global=$jscomp.getGlobal(this);$jscomp.polyfill=function(c,b,a,d){if(b){a=$jscomp.global;c=c.split(".");for(d=0;d<c.length-1;d++){var e=c[d];e in a||(a[e]={});a=a[e]}c=c[c.length-1];d=a[c];b=b(d);b!=d&&null!=b&&$jscomp.defineProperty(a,c,{configurable:!0,writable:!0,value:b})}};
$jscomp.polyfill("Object.values",function(c){return c?c:function(b){var a=[],c;for(c in b)$jscomp.owns(b,c)&&a.push(b[c]);return a}},"es8","es3");$jscomp.arrayIteratorImpl=function(c){var b=0;return function(){return b<c.length?{done:!1,value:c[b++]}:{done:!0}}};$jscomp.arrayIterator=function(c){return{next:$jscomp.arrayIteratorImpl(c)}};$jscomp.SYMBOL_PREFIX="jscomp_symbol_";$jscomp.initSymbol=function(){$jscomp.initSymbol=function(){};$jscomp.global.Symbol||($jscomp.global.Symbol=$jscomp.Symbol)};
...
function(b){$("#type-pane").hide();$("#prop-pane").hide();clean=!0;$("#legend-pane").hide();mobile_dev?$("#search-box").val(""):$("#search-box").focus().val("");offset=0;cur_neig=cur_node=null;$("#mapr-relmap").html("");$("#mapr-rel").html("");$("#mapr-reldiv").hide()}),sigma.parsers.json("maps.json",s,function(){filter=new sigma.plugins.filter(s);var b,a=s.graph.nodes(),c=a.length;for(b=0;b<c;b++)a[b].cat1viz=!0,a[b].cat2viz=!0,a[b].yearviz=!0;for(b=0;b<c;b++)a[b].color=a[b].center?"#333":"#666";
c=s.graph.edges();var e=c.length;for(b=0;b<e;b++)c[b].type="curve",c[b].defaultcolor=c[b].color;s.refresh();for(b=0;b<e;b++)"curved"==c[b].attributes[0]&&(c[b].type="curvedArrow");s.refresh();updateCountinfo(a.length);nolegendfornodes()}))})});var type_append=!1,prop_append=!1;offset=0;cur_neig=cur_node=null;var clean=!0,nextptr=1,ptrlen=0,neighbor;
...
$("#prev-btn").click(function(){nextptr--;nextptr--;0>nextptr&&(nextptr=ptrlen-1);$("#show-btn").click()});updatePane("#minor-pane",nodes[visnb[nextptr]],!1);$("#minor-pane").append("<hr>");$("#minor-pane").append("<p>"+edges[visedge[nextptr]].desc+"</p>");nextptr++}))}}function forceCleanNode(c){nodes=s.graph.nodes();for(j=0;j<nodes.length;j++)nodes[j].color="#666";s.refresh()}
function updateLegendbyNode(c){$("#red-info").html("<b><i>"+c.data.node.name+"</i></b>");$("#green-info").html("<b><i>"+c.data.node.name+"</i></b>");$("#blue-info").html("\u4e0e<b><i>"+c.data.node.name+"</i></b> \u76f8\u5173\u8054\u7684\u5176\u5b83\u6295\u5f71")}function updateLegendbyEdge(c,b){$("#red-info").html("<b><i>"+c.name+"</i></b>");$("#green-info").html("<b><i>"+b.name+"</i></b>")}
...
function catReset(){$("#main-pane").empty();$("#minor-pane").empty();var c=0;filter.undo("categories").nodesBy(function(b){b.cat1viz=!0;b.cat2viz=!0;b.yearviz&&c++;b.type="def";b.color="#666";return!0}).apply();updateCountinfo(c);nolegendfornodes();$("#cat-l1").val("all categories");$("#cat-l2").empty();$("#cat-l2").append("<option>no sub-categories</option>");$("#cat-l2").val("no sub-categories");$("#cat-more").val("== no more filters ==")}
function catFilter(c,b,a){var d=!1;for(i=0;i<Object.values(a.attributes).length;i++)Object.values(b).forEach(function(b,h){if("developable surfaces"==c)if("cylindrical"==a.attributes[i]?(d=!0,a.color=colormap[0]):"conic"==a.attributes[i]?(d=!0,a.color=colormap[1]):"azimuthal"==a.attributes[i]?(d=!0,a.color=colormap[2]):"pseudocylindrical"==a.attributes[i]?(d=!0,a.type="image",a.url="img/r,y.png"):"pseudoconic"==a.attributes[i]?(d=!0,a.type="image",a.url="img/g,y.png"):"pseudoazimuthal"==a.attributes[i]?
...
a.label)d=!0,a.type="image",a.url="img/ryk.png";else if("Strebe 1995"==a.label)d=!0,a.type="image",a.url="img/m-o.png";else{if("Ciric I"==a.label||"A4"==a.label||"Dedistort"==a.label)d=!0,a.color="#F0F"}else{"same shape"==c&&a.attributes[i].startsWith("same shape gp")&&(val=a.attributes[i].substring(13),d=!0,a.type="image",a.url="img/gp"+val+".png");if("similar shape"==c){a.attributes[i].startsWith("cylindrical equal-area")&&(d=!0,a.type="image",a.url="img/c,m.png");"Sinusoidal"==a.label&&(d=!0,a.type=
...
c){if("fusion-src"==a.attributes[i]||"blending-src"==a.attributes[i]||"const-src"==a.attributes[i])d=!0,a.type="def",a.color=colormap[0];if("Hammer"==a.label||"Aitoff"==a.label)a.type="image",a.url="img/r,m.png";"Mollweide"==a.label&&(a.type="image",a.url="img/r,c.png");"Collignon"==a.label&&(a.type="image",a.url="img/c,r.png")}a.attributes[i]==b&&(d=!0,"combinations"==c?(a.type="def",a.color=colormap[h+1]):"distortions"==c?"Sinusoidal"==a.label||"Werner"==a.label?(a.type="image",a.url="img/gpb,g.png"):
(a.type="def",a.color=colormap[h]):(a.type="def",a.color=colormap[h]))}});return{found:d,type:a.type,color:a.color,url:a.url}}
function catChange(){$("#main-pane").empty();$("#minor-pane").empty();nodes=s.graph.nodes();edges=s.graph.edges();sel=$(this).val();for(i=0;i<nodes.length;i++)nodes[i].cat1viz=!0;if("all categories"==sel)$("#cat-l2").empty(),$("#cat-l2").append("<option>no sub-categories</option>"),filter.undo("categories").nodesBy(function(a){return a.cat2viz},"categories").apply();else{var c=sub_categories.get(sel);$("#cat-l2").empty();$("#cat-l2").append("<option>all sub-categories</option>");Object.values(c).forEach(function(a){$("#cat-l2").append("<option>"+
a+"</option>")});var b=0;filter.undo("categories").nodesBy(function(a){var d=!1;ret=catFilter(sel,c,a);ret.found&&(d=!0);a.type=ret.type;a.color=ret.color;a.url=ret.url;a.type2=ret.type;a.color2=ret.color;a.url2=ret.url;d||(a.cat1viz=!1);d&&a.cat2viz&&b++;return d&&a.cat2viz&&a.yearviz},"categories").apply();updateCountinfo(b);updateColorinfo(sel,c)}}
function subcatChange(){$("#main-pane").empty();$("#minor-pane").empty();sel=$(this).val();idx=this.selectedIndex-1;for(i=0;i<nodes.length;i++)nodes[i].cat1viz=!0;if("all sub-categories"==sel){parsel=$("#cat-l1").val();var c=sub_categories.get(parsel),b=0;filter.undo("categories").nodesBy(function(a){var d=!1;ret=catFilter(parsel,c,a);ret.found&&(d=!0);a.type=ret.type;a.color=ret.color;a.url=ret.url;a.type2=ret.type;a.color2=ret.color;a.url2=ret.url;d||(a.cat1viz=!1);d&&a.cat2viz&&b++;return d&&a.cat2viz&&
a.yearviz},"categories").apply()}else b=0,filter.undo("categories").nodesBy(function(a){var c=!1;for(i=0;i<Object.values(a.attributes).length;i++){a.attributes[i]==sel&&(c=!0,a.type="def",a.color=colormap[idx],"fusion"==sel||"blending"==sel||"construction"==sel)&&(a.color=colormap[idx+1]);"cylindrical"==sel&&(a.color=colormap[0]);"conic"==sel&&(a.color=colormap[1]);"azimuthal"==sel&&(a.color=colormap[2]);"pseudocylindrical"==sel&&(a.type="image",a.url="img/r,y.png","Ortelius Oval"==a.label||"Apian Globular I"==
a.label||"Bacon Globular"==a.label)&&(c=!0,a.type="image",a.url="img/ryk.png");"pseudoconic"==sel&&(a.type="image",a.url="img/g,y.png");"polyconic"==sel&&"Strebe 1995"==a.label&&(a.type="image",a.url="img/m-o.png");if("pseudoazimuthal"==sel){a.type="image";a.url="img/mby.png";if("Hammer"==a.label||"Winkel 3"==a.label)c=!0,a.type="image",a.url=uselent?"img/mbok.png":"img/bok.png";if("Aitoff"==a.label||"Wagner 7"==a.label||"Wagner 8"==a.label||"Wagner 9"==a.label)a.type="image",a.url=uselent?"img/mbo.png":
"img/b,o.png"}"miscellaneous"==sel&&(a.color="#000","Hammer"==a.label||"Winkel 3"==a.label)&&(a.type="image",a.url=uselent?"img/mbok.png":"img/bok.png");"polyhedral"==sel&&(a.type="image",a.url="img/icosa.png");if("equal-area"==sel||"equidistant"==sel)if("Sinusoidal"==a.label||"Werner"==a.label)a.type="image",a.url="img/gpb,g.png";"other same shapes"==sel&&a.attributes[i].startsWith("same shape gp")&&(val=a.attributes[i].substring(13),c=!0,a.type="image",a.url="img/gp"+val+".png");if("similar shape"==
sel&&("Boggs eumorphic"==a.label||"Siemon IV"==a.label||"Putni\u0146\u0161 P1"==a.label||"Putni\u0146\u0161 P2"==a.label||"Putni\u0146\u0161 P3'"==a.label||"Putni\u0146\u0161 P4'"==a.label||"Wagner 1"==a.label||"Wagner 2"==a.label||"Wagner 3"==a.label)){"Boggs eumorphic"==a.label&&(c=!0,a.type="image",a.url="img/g-r.png");"Siemon IV"==a.label&&(c=!0,a.type="image",a.url="img/g-rk.png");if("Putni\u0146\u0161 P1"==a.label||"Putni\u0146\u0161 P2"==a.label)c=!0,a.type="image",a.url="img/gray-r.png";if("Putni\u0146\u0161 P3'"==
a.label||"Putni\u0146\u0161 P4'"==a.label)c=!0,a.type="image",a.url="img/gray-ok.png";if("Wagner 1"==a.label||"Wagner 2"==a.label||"Wagner 3"==a.label)c=!0,a.type="image",a.url="img/gray-o.png"}"(may be) interrupted"!=sel&&"Bonne group"!=sel||"Sinusoidal"!=a.label||(c=!0,a.type="image",a.url="img/gpb,g.png");"cylindrical equal-area family"!=sel&&"stretching"!=sel||!a.attributes[i].startsWith("cylindrical equal-area")||(c=!0,a.type="image",a.url="img/c,m.png");if("fusion"==sel&&"fusion-src"==a.attributes[i]||
"blending"==sel&&"blending-src"==a.attributes[i]||"construction"==sel&&"const-src"==a.attributes[i]){if("fusion-src"==a.attributes[i]||"blending-src"==a.attributes[i]||"const-src"==a.attributes[i])c=!0,a.type="def",a.color=colormap[0];if("Hammer"==a.label||"Aitoff"==a.label)a.type="image",a.url="img/r,m.png";"Mollweide"==a.label&&(a.type="image",a.url="img/r,c.png");"Collignon"==a.label&&(a.type="image",a.url="img/c,r.png")}}c||(a.cat1viz=!1);c&&a.cat2viz&&b++;return c&&a.cat2viz&&a.yearviz},"categories").apply();
updateCountinfo(b)}
...
"-pane").stop().fadeOut(3E3)});$(".popup-div").mouseover(function(){$(this).stop().fadeIn();return!1});$(".popup-div").mouseout(function(){$(this).stop().fadeOut(3E3);return!1});$("#link-issue-pane").load("./issue.html");$("#link-auth-pane").load("./copyright.html");$("#link-hist-pane").load("./history.html");$("#link-source-pane").load("./source.html");$("#link-help-pane").load("./help.html");$(window).resize(function(){s.refresh()});$("#checkbox-pane").css({left:.8333*$("#copy-pane").width()+15});
var c=$(document).height(),b=$(document).width();b<c?($("#tipfade").css({height:"150"}),$("#tipfade").html('<div style="text-align: center;position: relative;top:10%; font-size: 24px; color: #f4f0e4;">Warning: You may view on <span style="border: solid 1px;color: #f4f0e4;">mobile</span> device, some contents may be not optimized appropriately.</div>'),mobile_dev=!0,$("#search-box").attr("disabled","disabled"),$("#search-text").attr("title","Search box is disabled temporarily on mobile device."),$("<br>").insertBefore("#uselent"),
$("#button-pane").css({right:"10px"})):(1280>b||600>c)&&$("#tipfade").html('<div style="text-align: center;position: relative;top:10%; font-size: 24px; color: #f4f0e4;">Warning: Screen resolusion may be too <span style="border: solid 1px;color: #f4f0e4;">low</span> to render contents.</div>');1360!=b&&1366!=b&&1440!=b&&1536!=b&&1600!=b&&1680!=b||768!=c&&800!=c&&864!=c&&900!=c&&1024!=c&&1050!=c?$("#tipfade").hide().fadeIn(2E3).delay(3E3).fadeOut(3E3):$("#tipfade").hide()});
