...
a)return b;e(b);return c(b)})}if($jscomp.USE_PROXY_FOR_ES6_CONFORMANCE_CHECKS){if(b&&$jscomp.ES6_CONFORMANCE)return b}else if(c())return b;var f="$jscomp_hidden_"+Math.random();h("freeze");h("preventExtensions");h("seal");var k=0,l=function(a){this.id_=(k+=Math.random()+1).toString();if(a){a=$jscomp.makeIterator(a);for(var b;!(b=a.next()).done;)b=b.value,this.set(b[0],b[1])}};l.prototype.set=function(a,b){if(!d(a))throw Error("Invalid WeakMap key");e(a);if(!$jscomp.owns(a,f))throw Error("WeakMap key fail: "+
...
a.next=a.head=a},k=0;return d},"es6","es3");$jscomp.checkStringArgs=function(b,c,a){if(null==b)throw new TypeError("The 'this' value for String.prototype."+a+" must not be null or undefined");if(c instanceof RegExp)throw new TypeError("First argument to String.prototype."+a+" must not be a regular expression");return b+""};
$jscomp.polyfill("String.prototype.startsWith",function(b){return b?b:function(c,a){var b=$jscomp.checkStringArgs(this,c,"startsWith");c+="";var e=b.length,h=c.length;a=Math.max(0,Math.min(a|0,b.length));for(var f=0;f<h&&a<e;)if(b[a++]!=c[f++])return!1;return f>=h}},"es6","es3");var s,loaded=0,mobile_dev=!1;drawcvco=!0;uselent=!1;sigma.utils.pkg("sigma.canvas.nodes");
sigma.canvas.nodes.image=function(){var b={},c={},a={},d=function(a,c,d){var e=arguments,f=d("prefix")||"",h=a[f+"size"];a.color||d("defaultNodeColor");var m=a.url;b[m]?(c.save(),c.beginPath(),c.arc(a[f+"x"],a[f+"y"],a[f+"size"],0,2*Math.PI,!0),c.closePath(),c.clip(),c.drawImage(b[m],a[f+"x"]-h,a[f+"y"]-h,2*h,2*h),c.restore()):(sigma.canvas.nodes.image.cache(m),sigma.canvas.nodes.def.apply(sigma.canvas.nodes,e))};d.cache=function(d,h){h&&(a[d]=h);if(!c[d]){var e=new Image;e.onload=function(){c[d]=
...
colormap[3]+"''></div><div class='legend-text' style='display:inline-block'>&nbsp;&nbsp;&nbsp;cylindrical equal-area family</div><br>"),$("#colorinfo-pane").append("<div class='circle' style='display:inline-block;background-color:"+colormap[4]+"''></div><div class='legend-text' style='display:inline-block'>&nbsp;&nbsp;&nbsp;stretching</div><br>")):("combinations"==b?($("#colorinfo-pane").append("<div class='circle' style='display:inline-block;background-color:"+colormap[0]+"''></div><div class='legend-text' style='display:inline-block'>&nbsp;&nbsp;&nbsp;source</div><br>"),
a=1):a=0,Object.values(c).forEach(function(c,b){"McBryde or McBT"==c?$("#colorinfo-pane").append("<div class='circle' style='display:inline-block;background-color:"+colormap[b+a]+"''></div><div title = 'McBT denotes McBryde-Thomas' class='legend-text' style='display:inline-block'>&nbsp;&nbsp;&nbsp;"+c+"</div><br>"):"other same shapes"==c?($("#colorinfo-pane").append("<image src='img/qmark.png' height='20px' width='20px'><div class='legend-text' style='display:inline-block'>&nbsp;&nbsp;&nbsp;bi-color discs represent</div><br>"),
$("#colorinfo-pane").append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;other cases of the same shape")):$("#colorinfo-pane").append("<div class='circle' style='display:inline-block;background-color:"+colormap[b+a]+"''></div><div class='legend-text' style='display:inline-block'>&nbsp;&nbsp;&nbsp;"+c+"</div><br>")}))}
...
$("#amount").val(-a+"BC - "+c):$("#amount").val(a+" - "+c)});$("#slider-range").slider({range:!0,min:0,max:b.length-1,values:[0,b.length-1],slide:function(a,c){nodes=s.graph.nodes();edges=s.graph.edges();for(i=0;i<nodes.length;i++)nodes[i].yearviz=!0;$("#main-pane").empty();$("#minor-pane").empty();var d=b[c.values[0]],e=b[c.values[1]];0>e?$("#amount").val(-d+"BC - "+-e+"BC"):0>d?$("#amount").val(-d+"BC - "+e):$("#amount").val(d+" - "+e);var k=0;filter.undo("yearrange").nodesBy(function(a){var c=
!1;year=a.year;var b=/c.a./i;b.test(year)&&(year=year.replace("c.a.",""));year=parseInt(year);b=/BC/i;b.test(a.year)&&(year=-year);b=/and/i;b.test(a.year)?(split=a.year.split(" and "),y0=parseInt(split[0]),y1=parseInt(split[1]),y0>=d&&y0<=e&&(c=!0),y1>=d&&y1<=e&&(c=!0)):year>=d&&year<=e&&(c=!0);c&&a.cat1viz&&a.cat2viz&&k++;c||(a.yearviz=!1);return c},"yearrange").apply();updateCountinfo(k)}});var c=b[$("#slider-range").slider("values",0)],a=b[$("#slider-range").slider("values",1)];0>a?$("#amount").val(-c+
"BC - "+-a+"BC"):0>c?$("#amount").val(-c+"BC - "+a):$("#amount").val(c+" - "+a)});
...
c){c=c||"hidden";b=document.querySelectorAll(b);var a=b.length;for(i=0;i<a;i++){var d=b[i];-1!==d.className.indexOf(c)?d.className=d.className.replace(c,""):d.className+=" "+c}}},colormap="#c00 #0c0 #00c #c0c #0cc #cc0 #cff #ffc #cfc #ccc #ccc #ccc #333".split(" "),g={nodes:[],edges:[]};
...
function(c){$("#type-pane").hide();$("#prop-pane").hide();clean=!0;$("#legend-pane").hide();mobile_dev?$("#search-box").val(""):$("#search-box").focus().val("");offset=0;cur_neig=cur_node=null;$("#mapr-relmap").html("");$("#mapr-rel").html("");$("#mapr-reldiv").hide()}),sigma.parsers.json("maps.json",s,function(){filter=new sigma.plugins.filter(s);var c,a=s.graph.nodes(),b=a.length;for(c=0;c<b;c++)a[c].cat1viz=!0,a[c].cat2viz=!0,a[c].yearviz=!0;for(c=0;c<b;c++)a[c].color=a[c].center?"#333":"#666";
b=s.graph.edges();var e=b.length;for(c=0;c<e;c++)b[c].type="curve",b[c].defaultcolor=b[c].color;s.refresh();for(c=0;c<e;c++)"curved"==b[c].attributes[0]&&(b[c].type="curvedArrow");s.refresh();updateCountinfo(a.length);nolegendfornodes()}))})});var type_append=!1,prop_append=!1;offset=0;cur_neig=cur_node=null;var clean=!0,nextptr=1,ptrlen=0,neighbor;
function updatePane(b,c,a){nodes=s.graph.nodes();edges=s.graph.edges();$(b).empty();mobile_dev&&$(b).css({top:parseInt($("#cat-l1").css("height"))+20+"px"});c.name?$(b).append('<h3 class="underline">'+c.name+"</h3>"):$(b).append('<h3 class="underline">'+c.label+" projection</h3>");c.alias&&$(b).append("<p>Alias: "+c.alias+"</p>");c.crby&&(c.year?$(b).append("<p>Created by "+c.crby+" in "+c.year+"</p>"):$(b).append("<p>Created by "+c.crby+"</p>"));var d="",e="";for(i=0;i<Object.values(c.attributes).length;i++){if("conformal"==
c.attributes[i]||"equal-area"==c.attributes[i]||"equidistant"==c.attributes[i]||"compromise"==c.attributes[i])e=e?e+(", "+c.attributes[i]):e+c.attributes[i];if("cylindrical"==c.attributes[i]||"pseudocylindrical"==c.attributes[i]||"conic"==c.attributes[i]||"pseudoconic"==c.attributes[i]||"azimuthal"==c.attributes[i]||"lenticular"==c.attributes[i])d+=c.attributes[i]}c.img&&$(b).append('<img src="'+c.img+'" width=100%></img>');d&&$(b).append("<p>Type: "+d+"</p>");e&&$(b).append("<p>Properties: "+e+"</p>");
$(b).show();if(a){nextptr=0;neighbor=c.neighbors;for(j=0;j<edges.length;j++)edges[j].color="#000";visnb=[];visedge=[];c.edges.forEach(function(a,b){0!=b&&(edges[a].color=edges[a].defaultcolor,vis=nodes[c.neighbors[b]].cat1viz&&nodes[c.neighbors[b]].cat2viz&&nodes[c.neighbors[b]].yearviz)&&(visnb.push(c.neighbors[b]),visedge.push(c.edges[b]))});ptrlen=Object.values(visedge).length;0!=ptrlen&&($(b).append('<hr><div id="show-rel" style="text-align:center"><button id="show-btn">show relations ('+ptrlen+
")</button></div>"),$(b).append('<div id="rel-area"</div>'),s.refresh(),$("#show-btn").click(function(){1<ptrlen?$("#show-btn").text("next relations"):$("#show-btn").hide();for(j=0;j<edges.length;j++)edges[j].color="#000";nextptr>=ptrlen&&(nextptr=0);edges[visedge[nextptr]].color=edges[visedge[nextptr]].defaultcolor;s.refresh();$("#rel-area").empty();$("#rel-area").append("<p><< see <b>"+nodes[visnb[nextptr]].label+" projection</b> ("+(nextptr+1)+"/"+ptrlen+") in left side panel</p>");1<ptrlen&&$("#rel-area").append('<div style="text-align:center"><button id="prev-btn">prev relations</button></div>');
...
function catReset(){$("#main-pane").empty();$("#minor-pane").empty();filter.undo("categories").nodesBy(function(b){b.type="def";b.color="#666";return!0}).apply();updateCountinfo(nodes.length);nolegendfornodes();$("#cat-l1").val("all categories");$("#cat-l2").empty();$("#cat-l2").append("<option>no sub-categories</option>");$("#cat-l2").val("no sub-categories");$("#cat-more").val("== no more filters ==")}
function catFilter(b,c,a){var d=!1;for(i=0;i<Object.values(a.attributes).length;i++)Object.values(c).forEach(function(c,h){if("developable surfaces"==b)if("cylindrical"==a.attributes[i]?(d=!0,a.color=colormap[0]):"conic"==a.attributes[i]?(d=!0,a.color=colormap[1]):"azimuthal"==a.attributes[i]?(d=!0,a.color=colormap[2]):"pseudocylindrical"==a.attributes[i]?(d=!0,a.type="image",a.url="img/r,y.png"):"pseudoconic"==a.attributes[i]?(d=!0,a.type="image",a.url="img/g,y.png"):"pseudoazimuthal"==a.attributes[i]?
(d=!0,a.type="image",a.url="img/mby.png"):"miscellaneous"==a.attributes[i]?(d=!0,a.color="#000"):"polyhedral"==a.attributes[i]&&(d=!0,a.type="image",a.url="img/icosa.png"),"Aitoff"==a.label||"Wagner 7"==a.label||"Wagner 8"==a.label||"Wagner 9"==a.label)d=!0,a.type="image",a.url=uselent?"img/mbo.png":"img/b,o.png";else if("Hammer"==a.label||"Winkel 3"==a.label)d=!0,a.type="image",a.url=uselent?"img/mbok.png":"img/bok.png";else if("Ortelius Oval"==a.label||"Apian Globular I"==a.label||"Bacon Globular"==
a.label)d=!0,a.type="image",a.url="img/ryk.png";else if("Strebe 1995"==a.label)d=!0,a.type="image",a.url="img/m-o.png";else{if("Ciric I"==a.label||"A4"==a.label||"Dedistort"==a.label)d=!0,a.color="#F0F"}else{"same shape"==b&&a.attributes[i].startsWith("same shape gp")&&(val=a.attributes[i].substring(13),d=!0,a.type="image",a.url="img/gp"+val+".png");if("similar shape"==b){a.attributes[i].startsWith("cylindrical equal-area")&&(d=!0,a.type="image",a.url="img/c,m.png");"Sinusoidal"==a.label&&(d=!0,a.type=
"image",a.url="img/gpb,g.png");"Boggs eumorphic"==a.label&&(d=!0,a.type="image",a.url="img/g-r.png");"Siemon IV"==a.label&&(d=!0,a.type="image",a.url="img/g-rk.png");if("Putni\u0146\u0161 P1"==a.label||"Putni\u0146\u0161 P2"==a.label)d=!0,a.type="image",a.url="img/gray-r.png";if("Putni\u0146\u0161 P3'"==a.label||"Putni\u0146\u0161 P4'"==a.label)d=!0,a.type="image",a.url="img/gray-ok.png";if("Wagner 1"==a.label||"Wagner 2"==a.label||"Wagner 3"==a.label)d=!0,a.type="image",a.url="img/gray-o.png"}if("combinations"==
b){if("fusion-src"==a.attributes[i]||"blending-src"==a.attributes[i]||"const-src"==a.attributes[i])d=!0,a.type="def",a.color=colormap[0];if("Hammer"==a.label||"Aitoff"==a.label)a.type="image",a.url="img/r,m.png";"Mollweide"==a.label&&(a.type="image",a.url="img/r,c.png");"Collignon"==a.label&&(a.type="image",a.url="img/c,r.png")}a.attributes[i]==c&&(d=!0,"combinations"==b?(a.type="def",a.color=colormap[h+1]):"distortions"==b?"Sinusoidal"==a.label||"Werner"==a.label?(a.type="image",a.url="img/gpb,g.png"):
(a.type="def",a.color=colormap[h]):(a.type="def",a.color=colormap[h]))}});return{found:d,type:a.type,color:a.color,url:a.url}}
...
a+"</option>")});var c=0;filter.undo("categories").nodesBy(function(a){var d=!1;ret=catFilter(sel,b,a);ret.found&&(d=!0);a.type=ret.type;a.color=ret.color;a.url=ret.url;a.type2=ret.type;a.color2=ret.color;a.url2=ret.url;d||(a.cat1viz=!1);d&&a.cat2viz&&c++;return d&&a.cat2viz&&a.yearviz},"categories").apply();updateCountinfo(c);updateColorinfo(sel,b)}}
function subcatChange(){$("#main-pane").empty();$("#minor-pane").empty();sel=$(this).val();idx=this.selectedIndex-1;for(i=0;i<nodes.length;i++)nodes[i].cat1viz=!0;if("all sub-categories"==sel){parsel=$("#cat-l1").val();var b=sub_categories.get(parsel),c=0;filter.undo("categories").nodesBy(function(a){var d=!1;ret=catFilter(parsel,b,a);ret.found&&(d=!0);a.type=ret.type;a.color=ret.color;a.url=ret.url;a.type2=ret.type;a.color2=ret.color;a.url2=ret.url;d||(a.cat1viz=!1);d&&a.cat2viz&&c++;return d&&a.cat2viz&&
a.yearviz},"categories").apply()}else c=0,filter.undo("categories").nodesBy(function(a){var b=!1;for(i=0;i<Object.values(a.attributes).length;i++){a.attributes[i]==sel&&(b=!0,a.type="def",a.color=colormap[idx],"fusion"==sel||"blending"==sel||"construction"==sel)&&(a.color=colormap[idx+1]);"cylindrical"==sel&&(a.color=colormap[0]);"conic"==sel&&(a.color=colormap[1]);"azimuthal"==sel&&(a.color=colormap[2]);"pseudocylindrical"==sel&&(a.type="image",a.url="img/r,y.png","Ortelius Oval"==a.label||"Apian Globular I"==
a.label||"Bacon Globular"==a.label)&&(b=!0,a.type="image",a.url="img/ryk.png");"pseudoconic"==sel&&(a.type="image",a.url="img/g,y.png");"polyconic"==sel&&"Strebe 1995"==a.label&&(a.type="image",a.url="img/m-o.png");if("pseudoazimuthal"==sel){a.type="image";a.url="img/mby.png";if("Hammer"==a.label||"Winkel 3"==a.label)b=!0,a.type="image",a.url=uselent?"img/mbok.png":"img/bok.png";if("Aitoff"==a.label||"Wagner 7"==a.label||"Wagner 8"==a.label||"Wagner 9"==a.label)a.type="image",a.url=uselent?"img/mbo.png":
"img/b,o.png"}"miscellaneous"==sel&&(a.color="#000","Hammer"==a.label||"Winkel 3"==a.label)&&(a.type="image",a.url=uselent?"img/mbok.png":"img/bok.png");"polyhedral"==sel&&(a.type="image",a.url="img/icosa.png");if("equal-area"==sel||"equidistant"==sel)if("Sinusoidal"==a.label||"Werner"==a.label)a.type="image",a.url="img/gpb,g.png";"other same shapes"==sel&&a.attributes[i].startsWith("same shape gp")&&(val=a.attributes[i].substring(13),b=!0,a.type="image",a.url="img/gp"+val+".png");if("similar shape"==
sel&&("Boggs eumorphic"==a.label||"Siemon IV"==a.label||"Putni\u0146\u0161 P1"==a.label||"Putni\u0146\u0161 P2"==a.label||"Putni\u0146\u0161 P3'"==a.label||"Putni\u0146\u0161 P4'"==a.label||"Wagner 1"==a.label||"Wagner 2"==a.label||"Wagner 3"==a.label)){"Boggs eumorphic"==a.label&&(b=!0,a.type="image",a.url="img/g-r.png");"Siemon IV"==a.label&&(b=!0,a.type="image",a.url="img/g-rk.png");if("Putni\u0146\u0161 P1"==a.label||"Putni\u0146\u0161 P2"==a.label)b=!0,a.type="image",a.url="img/gray-r.png";if("Putni\u0146\u0161 P3'"==
a.label||"Putni\u0146\u0161 P4'"==a.label)b=!0,a.type="image",a.url="img/gray-ok.png";if("Wagner 1"==a.label||"Wagner 2"==a.label||"Wagner 3"==a.label)b=!0,a.type="image",a.url="img/gray-o.png"}"(may be) interrupted"!=sel&&"Bonne group"!=sel||"Sinusoidal"!=a.label||(b=!0,a.type="image",a.url="img/gpb,g.png");"cylindrical equal-area family"!=sel&&"stretching"!=sel||!a.attributes[i].startsWith("cylindrical equal-area")||(b=!0,a.type="image",a.url="img/c,m.png");if("fusion"==sel&&"fusion-src"==a.attributes[i]||
"blending"==sel&&"blending-src"==a.attributes[i]||"construction"==sel&&"const-src"==a.attributes[i]){if("fusion-src"==a.attributes[i]||"blending-src"==a.attributes[i]||"const-src"==a.attributes[i])b=!0,a.type="def",a.color=colormap[0];if("Hammer"==a.label||"Aitoff"==a.label)a.type="image",a.url="img/r,m.png";"Mollweide"==a.label&&(a.type="image",a.url="img/r,c.png");"Collignon"==a.label&&(a.type="image",a.url="img/c,r.png")}}b||(a.cat1viz=!1);b&&a.cat2viz&&c++;return b&&a.cat2viz&&a.yearviz},"categories").apply();
updateCountinfo(c)}
function morecatChange(){nodes=s.graph.nodes();edges=s.graph.edges();sel=$(this).val();for(i=0;i<nodes.length;i++)nodes[i].cat2viz=!0;if("== no more filters =="==sel){var b=0;filter.undo("categories").nodesBy(function(c){c.type=c.type2;c.color=c.color2;c.url=c.url2;c.cat1viz&&b++;return c.cat1viz},"categories").apply()}else b=0,filter.undo("categories").nodesBy(function(c){var a=!1;c.type=c.type2;c.color=c.color2;c.url=c.url2;for(i=0;i<Object.values(c.attributes).length;i++)c.attributes[i]==sel&&
(a=!0);a||(c.cat2viz=!1);a&&c.cat1viz&&b++;return a&&c.cat1viz&&c.yearviz},"categories").apply();updateCountinfo(b);s.refresh()}function search(){var b=$(this).val();b=new RegExp(b,"i");$(this).val("");forceCleanNode();nodes=s.graph.nodes();var c=!1;for(j=0;j<nodes.length;j++)b.test(nodes[j].label)&&(nodes[j].color="#c00",c=!0);c?s.refresh():$(this).val("no result found!").select()}
$(document).ready(function(){$("#red-circle").css("background","#c00");$("#blue-circle").css("background","#00c");$("#green-circle").css("background","#0c0");$("#search-pane").css("left",parseInt($("#search-pane").css("right"))/2);$("#search-box").bind("change",search).focus();$("#legend-pane").hide();$("#cat-l1").change(catChange);$("#cat-l2").change(subcatChange);$("#cat-more").change(morecatChange);$("#reset-all").click(catReset);$("#zoom-vis").click(function(){s.refresh()});$("#uselent").change(function(){uselent=
!uselent;$("#cat-more").empty();$("#cat-more").append("<option>== no more filters ==</option>");$("#cat-more").append("<option>cylindrical</option>");$("#cat-more").append("<option>pseudocylindrical</option>");$("#cat-more").append("<option>conic</option>");$("#cat-more").append("<option>pseudoconic</option>");$("#cat-more").append("<option>azimuthal</option>");uselent&&$("#cat-more").append("<option>lenticular</option>");$("#cat-more").append("<option>== no more filters ==</option>");$("#cat-more").append("<option>conformal</option>");
$("#cat-more").append("<option>equal-area</option>");$("#cat-more").append("<option>equidistant</option>");$("#cat-more").append("<option>compromise</option>");$("#reset-all").click();uselent?sub_categories.set("developable surfaces","cylindrical conic azimuthal pseudocylindrical pseudoconic polyconic lenticular miscellaneous".split(" ")):sub_categories.set("developable surfaces","cylindrical conic azimuthal pseudocylindrical pseudoconic polyconic miscellaneous".split(" "))});$("#drawcvco").change(function(){nodes=
s.graph.nodes();edges=s.graph.edges();if(drawcvco=!drawcvco)for(j=0;j<edges.length;j++)edges[j].color=edges[j].defaultcolor;else for(j=0;j<edges.length;j++)edges[j].color="#000";s.refresh()});$(".link").mouseover(function(a){$(".popup-div").not("#"+$(this).attr("id")+"-pane").hide();b=parseInt($(this).attr("height"));$("#"+$(this).attr("id")+"-pane").css({position:"absolute",top:a.pageY-20*b,left:a.pageX+10});$("#"+$(this).attr("id")+"-pane").stop().show()});$(".link").mouseout(function(){$("#"+$(this).attr("id")+
"-pane").stop().fadeOut(3E3)});$(".popup-div").mouseover(function(){$(this).stop().fadeIn();return!1});$(".popup-div").mouseout(function(){$(this).stop().fadeOut(3E3);return!1});$("#link-issue-pane").load("./issue.html");$("#link-auth-pane").load("./copyright.html");$("#link-hist-pane").load("./history.html");$("#link-source-pane").load("./source.html");$("#link-help-pane").load("./help.html");$(window).resize(function(){s.refresh()});var b=$(document).height(),c=$(document).width();c<b?($("#tipfade").css({height:"150"}),
$("#tipfade").html('<div style="text-align: center;position: relative;top:10%; font-size: 24px; color: #f4f0e4;">Warning: You may view on <span style="border: solid 1px;color: #f4f0e4;">mobile</span> device, some contents may be not optimized appropriately.</div>'),mobile_dev=!0,$("#search-box").attr("disabled","disabled"),$("#search-text").attr("title","Search box is disabled temporarily on mobile device."),$("<br>").insertBefore("#uselent"),$("#button-pane").css({right:"10px"})):(1280>c||600>b)&&
$("#tipfade").html('<div style="text-align: center;position: relative;top:10%; font-size: 24px; color: #f4f0e4;">Warning: Screen resolusion may be too <span style="border: solid 1px;color: #f4f0e4;">low</span> to render contents.</div>');1360!=c&&1366!=c&&1440!=c&&1536!=c&&1600!=c&&1680!=c||768!=b&&800!=b&&864!=b&&900!=b&&1024!=b&&1050!=b?$("#tipfade").hide().fadeIn(2E3).delay(3E3).fadeOut(3E3):$("#tipfade").hide()});
