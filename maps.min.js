...
!1;year=a.year;var b=/c.a./i;b.test(year)&&(year=year.replace("c.a.",""));year=parseInt(year);b=/BC/i;b.test(a.year)&&(year=-year);b=/and/i;b.test(a.year)?(split=a.year.split(" and "),y0=parseInt(split[0]),y1=parseInt(split[1]),y0>=d&&y0<=e&&(c=!0),y1>=d&&y1<=e&&(c=!0)):year>=d&&year<=e&&(c=!0);c&&k++;c||(a.yearviz=!1);return c},"yearrange").apply();updateCountinfo(k)}});var c=b[$("#slider-range").slider("values",0)],a=b[$("#slider-range").slider("values",1)];0>a?$("#amount").val(-c+"BC - "+-a+"BC"):
0>c?$("#amount").val(-c+"BC - "+a):$("#amount").val(c+" - "+a)});
...
a=s.graph.edges();b=a.length;for(c=0;c<b;c++)a[c].type="curve",a[c].defaultcolor=a[c].color;$("#uselent").change();s.refresh();for(c=0;c<b;c++)"curved"==a[c].attributes[0]&&(a[c].type="curvedArrow");s.refresh();updateCountinfo(102);nolegendfornodes()}))})});var type_append=!1,prop_append=!1;offset=0;cur_neig=cur_node=null;var clean=!0,nextptr=1,ptrlen=0,neighbor;
