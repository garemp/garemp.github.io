...
$(function(){var c=[];genarray(c,-300,1500,50);genarray(c,1500,1800,15);genarray(c,1800,1900,2);genarray(c,1900,2021,1);$("#reset-year").click(function(){$("#comp-div").empty().hide();nodes=s.graph.nodes();edges=s.graph.edges();var a=0;for(i=0;i<nodes.length;i++)nodes[i].yearviz=!0,nodes[i].cat1viz&&nodes[i].cat2viz&&a++;filter.undo("yearrange").apply();updateCountinfo(a);$("#slider-range").slider("values",0,0);$("#slider-range").slider("values",1,c.length-1);a=c[$("#slider-range").slider("values",
0)];var b=c[$("#slider-range").slider("values",1)];0>b?$("#amount").val(-a+"BC - "+-b+"BC"):0>a?$("#amount").val(-a+"BC - "+b):$("#amount").val(a+" - "+b)});$("#slider-range").slider({range:!0,min:0,max:c.length-1,values:[0,c.length-1],slide:function(a,b){$("#comp-div").empty().hide();nodes=s.graph.nodes();edges=s.graph.edges();for(i=0;i<nodes.length;i++)nodes[i].yearviz=!0;$("#main-pane").empty();$("#minor-pane").empty();var d=c[b.values[0]],e=c[b.values[1]];0>e?$("#amount").val(-d+"BC - "+-e+"BC"):
0>d?$("#amount").val(-d+"BC - "+e):$("#amount").val(d+" - "+e);var k=0;filter.undo("yearrange").nodesBy(function(a){var b=!1;year=a.year;var c=/c.a./i;c.test(year)&&(year=year.replace("c.a.",""));year=parseInt(year);c=/BC/i;c.test(a.year)&&(year=-year);c=/and/i;c.test(a.year)?(split=a.year.split(" and "),y0=parseInt(split[0]),y1=parseInt(split[1]),y0>=d&&y0<=e&&(b=!0),y1>=d&&y1<=e&&(b=!0)):year>=d&&year<=e&&(b=!0);b&&a.cat1viz&&a.cat2viz&&k++;b||(a.yearviz=!1);return b},"yearrange").apply();updateCountinfo(k)}});
var b=c[$("#slider-range").slider("values",0)],a=c[$("#slider-range").slider("values",1)];0>a?$("#amount").val(-b+"BC - "+-a+"BC"):0>b?$("#amount").val(-b+"BC - "+a):$("#amount").val(b+" - "+a)});
...
urls.forEach(function(c){sigma.canvas.nodes.image.cache(c,function(){++loaded===urls.length&&(s=new sigma({graph:g,container:"graph-container",renderer:{container:document.getElementById("graph-container"),type:"canvas"},settings:{minNodeSize:8,maxNodeSize:16,enableEdgeHovering:!0,edgeHoverColor:"edge",defaultEdgeHoverColor:"#000",edgeHoverSizeRatio:1,edgeHoverExtremities:!0}}),s.bind("clickNode",function(b){$("#comp-div").empty().hide();$("#minor-pane").hide();drawcvco=0;$("#drawcvco").removeAttr("checked");
node=b.data.node;updatePane("#main-pane",node,!0)}),s.bind("clickEdge",function(b){$("#comp-div").empty().hide();nodes=s.graph.nodes();edges=s.graph.edges();node1=nodes[b.data.edge.s];node2=nodes[b.data.edge.t];updatePane("#main-pane",node1,!1);updatePane("#minor-pane",node2,!1);$("#minor-pane").append("<hr>");$("#minor-pane").append("<p>"+b.data.edge.desc+"</p>");drawcvco=0;$("#drawcvco").removeAttr("checked");for(j=0;j<edges.length;j++)edges[j].color=b.data.edge.id==edges[j].id?edges[j].defaultcolor:
"#000";$("#main-pane").append("<hr><p><< see <b>"+node2.label+' projection</b> in left side panel&nbsp;<button id="comp-btn">compare!</button></div></p>');compareMap(node1,node2);s.refresh()}),s.bind("clickStage",function(b){$("#type-pane").hide();$("#prop-pane").hide();clean=!0;mobile_dev?$("#search-box").val(""):$("#search-box").focus().val("");offset=0;cur_neig=cur_node=null;$("#mapr-relmap").html("");$("#mapr-rel").html("");$("#mapr-reldiv").hide()}),sigma.parsers.json("maps.json",s,function(){filter=
new sigma.plugins.filter(s);var b,a=s.graph.nodes(),c=a.length;for(b=0;b<c;b++)a[b].cat1viz=!0,a[b].cat2viz=!0,a[b].yearviz=!0,"Quartic authalic"==a[b].label&&(a[b].y-=8),"SAND 2000"==a[b].label&&(a[b].y-=8),"Hobo-Dyer"==a[b].label&&(a[b].y-=8),"HEALPix"==a[b].label&&(a[b].y-=5),"Strebe 1995"==a[b].label&&(a[b].y+=8),"Azimuthal equidistant"==a[b].label&&(a[b].x+=5,a[b].y+=5),"Wagner 9"==a[b].label&&(a[b].x-=18),"Hammer"==a[b].label&&(a[b].dy+=3),"Aitoff"==a[b].label&&(a[b].dy-=5),"Lambert cylindrical"==
a[b].label&&(a[b].dy-=5);for(b=0;b<c;b++)a[b].size=7,a[b].color=a[b].center?"#333":"#AAA",a[b].colorf="#AAA",a[b].colorg="#AAA";c=s.graph.edges();var e=c.length;for(b=0;b<e;b++)c[b].type="curve",c[b].defaultcolor=c[b].color;s.refresh();for(b=0;b<e;b++)"curved"==c[b].attributes[0]&&(c[b].type="curvedArrow");s.refresh();updateCountinfo(a.length);nolegendfornodes()}))})});var type_append=!1,prop_append=!1;offset=0;cur_neig=cur_node=null;var clean=!0,nextptr=1,ptrlen=0,neighbor;
function compareMap(c,b){var a=c.img,d=b.img;var e=c.name?c.name:c.label+" projection";var h=b.name?b.name:b.label+" projection";e="(click image to toggle projections) >> "+e;h="(click image to toggle projections) >> "+h;$("#comp-div").empty().hide();$("#comp-btn").click(function(){$(this).attr("disabled",!0);$("#comp-div").append("<span id='comp-name'>"+e+"</span><img id='comp-close' width='20px' style='float: right;' src='img/cross.svg'><br><img id='comp-img' width='600px' style='position:relative' src="+
a+"></img>");$("#comp-img").waitForImages(!0).done(function(){300<$("#comp-img").height()?($("#comp-img").attr("height","300px"),$("#comp-img").removeAttr("width"),$("#comp-img").css({top:"0px"}),$("#comp-img").css({left:(600-$("#comp-img").width())/2+"px"})):($("#comp-img").css({left:"0px"}),$("#comp-img").css({top:(300-$(this).height())/2+"px"}))});$("#comp-div").show().css({top:"100px"});$("#comp-img").click(function(){$(this).removeAttr("height");$(this).attr("src")==a?($("#comp-name").text(h),
$(this).attr("src",d)):($("#comp-name").text(e),$(this).attr("src",a));$(this).attr("width","600px");300<$(this).height()?($(this).attr("height","300px"),$(this).removeAttr("width"),$(this).css({top:"0px"}),$(this).css({left:(600-$(this).width())/2+"px"})):($(this).css({left:"0px"}),$(this).css({top:(300-$(this).height())/2+"px"}))});$("#comp-close").click(function(){$("#comp-div").empty().hide();$("#comp-btn").attr("disabled",!1)})})}
...
b.attributes[i]||"equal-area"==b.attributes[i]||"equidistant"==b.attributes[i]||"compromise"==b.attributes[i])e=e?e+(", "+b.attributes[i]):e+b.attributes[i];if("cylindrical"==b.attributes[i]||"pseudocylindrical"==b.attributes[i]||"conic"==b.attributes[i]||"pseudoconic"==b.attributes[i]||"azimuthal"==b.attributes[i]||"lenticular"==b.attributes[i])d+=b.attributes[i]}b.img&&$(c).append("<img id='"+c.substring(1)+"img' src=\""+b.img+'" width=100%></img>');$(c+"img").waitForImages(!0).done(function(){20==
$(this).height()&&$(this).attr("title","No preview image available")});d&&$(c).append("<p>Type: "+d+"</p>");e&&$(c).append("<p>Properties: "+e+"</p>");$(c).show();if(a){nextptr=0;neighbor=b.neighbors;for(j=0;j<edges.length;j++)edges[j].color="#000";visnb=[];visedge=[];b.edges.forEach(function(a,c){0!=c&&(edges[a].color=edges[a].defaultcolor,vis=nodes[b.neighbors[c]].cat1viz&&nodes[b.neighbors[c]].cat2viz&&nodes[b.neighbors[c]].yearviz)&&(visnb.push(b.neighbors[c]),visedge.push(b.edges[c]))});ptrlen=
Object.values(visedge).length;0!=ptrlen&&($(c).append('<hr><div id="show-rel" style="text-align:center"><button id="show-btn">show relations ('+ptrlen+")</button></div>"),$(c).append('<div id="rel-area"</div>'),s.refresh(),$("#show-btn").click(function(){1<ptrlen?$("#show-btn").text("next relations"):$("#show-btn").hide();for(j=0;j<edges.length;j++)edges[j].color="#000";nextptr>=ptrlen&&(nextptr=0);edges[visedge[nextptr]].color=edges[visedge[nextptr]].defaultcolor;s.refresh();$("#rel-area").empty();
$("#rel-area").append("<p><< see <b>"+nodes[visnb[nextptr]].label+" projection</b> ("+(nextptr+1)+"/"+ptrlen+') in left side panel&nbsp;<button id="comp-btn">compare!</button></div></p>');1<ptrlen&&$("#rel-area").append('<div style="text-align:center"><button id="prev-btn">prev relations</button></div>');compareMap(b,nodes[visnb[nextptr]]);$("#prev-btn").click(function(){nextptr--;nextptr--;0>nextptr&&(nextptr=ptrlen-1);$("#show-btn").click();$("#comp-div").empty().hide()});updatePane("#minor-pane",
nodes[visnb[nextptr]],!1);$("#minor-pane").append("<hr>");$("#minor-pane").append("<p>"+edges[visedge[nextptr]].desc+"</p>");nextptr++}))}}function forceCleanNode(c){nodes=s.graph.nodes();for(j=0;j<nodes.length;j++)nodes[j].color="#AAA";s.refresh()}function nolegendfornodes(){$("#colorinfo-pane").empty();mobile_dev&&$("#color-count-info-pane").css({bottom:parseInt($("#drawcvco").css("height"))+20+"px"});$("#colorinfo-pane").append("<div id='legend-title'><b>No Legend for Nodes</b></div>")}
var sub_categories=new Map;sub_categories.set("developable surfaces","cylindrical conic azimuthal pseudocylindrical pseudoconic polyconic lenticular miscellaneous".split(" "));sub_categories.set("distortions",["conformal","equal-area","equidistant","compromise"]);sub_categories.set("aspects (partial)",["normal","transverse","oblique"]);sub_categories.set("same shape",["2:1 ellipse","other same shapes"]);
sub_categories.set("similar shape",["similar shape","(may be) interrupted","Bonne group","cylindrical equal-area family","stretching"]);sub_categories.set("form of parallels",["evenly spaced"]);sub_categories.set("form of meridians",["elliptical","sinusoidal","quartic","parabolic","hyperbolic"]);sub_categories.set("form of poles",["as straight lines","as points"]);sub_categories.set("combinations",["fusion","blending","construction","Tobler hyperelliptical family"]);
sub_categories.set("authors (inc. aliases)","Eckert;Kavrayskiy;Lambert;McBryde or McBT;Putni\u0146\u0161;Siemon;Strebe;Tobler;Wagner;Werenskiold;Winkel;Hufnagel's family".split(";"));
function catReset(){$("#comp-div").empty().hide();$("#main-pane").empty();$("#minor-pane").empty();var c=0;filter.undo("categories").nodesBy(function(b){b.cat1viz=!0;b.cat2viz=!0;b.yearviz&&c++;b.type="def";b.color="#AAA";b.colorf="#AAA";b.colorg="#AAA";return!0},"categories").apply();updateCountinfo(c);nolegendfornodes();$("#cat-l1").val("all categories");$("#cat-l2").empty();$("#cat-l2").append("<option>no sub-categories</option>");$("#cat-l2").val("no sub-categories");$("#cat-more").val("== no more filters ==")}
...
function catChange(){$("#comp-div").empty().hide();$("#main-pane").empty();$("#minor-pane").empty();nodes=s.graph.nodes();edges=s.graph.edges();sel=$(this).val();for(i=0;i<nodes.length;i++)nodes[i].cat1viz=!0;if("all categories"==sel)$("#cat-l2").empty(),$("#cat-l2").append("<option>no sub-categories</option>"),filter.undo("categories").nodesBy(function(a){a.type="def";a.type2=a.type;a.color="#AAA";a.color2=a.color;a.colorf=a.color;a.colorg=a.color;return a.cat2viz},"categories").apply(),updateCountinfo(nodes.length),
nolegendfornodes();else{var c=sub_categories.get(sel);$("#cat-l2").empty();$("#cat-l2").append("<option>all sub-categories</option>");Object.values(c).forEach(function(a){$("#cat-l2").append("<option>"+a+"</option>")});var b=0;filter.undo("categories").nodesBy(function(a){var d=!1;ret=catFilter(sel,c,a);ret.found&&(d=!0);a.type=ret.type;a.color=ret.color;a.url=ret.url;a.type2=ret.type;a.color2=ret.color;a.url2=ret.url;a.colorf=ret.colorf;a.colorg=ret.colorg;d||(a.cat1viz=!1);d&&a.cat2viz&&b++;return d&&
a.cat2viz&&a.yearviz},"categories").apply();updateCountinfo(b);updateColorinfo(sel,c)}}
function subcatChange(){$("#comp-div").empty().hide();$("#main-pane").empty();$("#minor-pane").empty();sel=$(this).val();idx=this.selectedIndex-1;for(i=0;i<nodes.length;i++)nodes[i].cat1viz=!0;if("all sub-categories"==sel){parsel=$("#cat-l1").val();var c=sub_categories.get(parsel),b=0;filter.undo("categories").nodesBy(function(a){var d=!1;ret=catFilter(parsel,c,a);ret.found&&(d=!0);a.type=ret.type;a.color=ret.color;a.url=ret.url;a.type2=ret.type;a.color2=ret.color;a.url2=ret.url;a.colorf=ret.colorf;
a.colorg=ret.colorg;d||(a.cat1viz=!1);d&&a.cat2viz&&b++;return d&&a.cat2viz&&a.yearviz},"categories").apply()}else b=0,filter.undo("categories").nodesBy(function(a){var c=!1;for(i=0;i<Object.values(a.attributes).length;i++){a.attributes[i]==sel&&(c=!0,a.type="def",a.color=colormap[idx],"fusion"==sel||"blending"==sel||"construction"==sel)&&(a.color=colormap[idx+1]);"cylindrical"==sel&&(a.color=colormap[0]);"conic"==sel&&(a.color=colormap[1]);"azimuthal"==sel&&(a.color=colormap[2]);"pseudocylindrical"==
sel&&(a.type="image",a.url="img/r,y.png","Ortelius Oval"==a.label||"Apian Globular I"==a.label||"Bacon Globular"==a.label)&&(c=!0,a.type="image",a.url="img/ryk.png");"pseudoconic"==sel&&(a.type="image",a.url="img/g,y.png");"polyconic"==sel&&"Strebe 1995"==a.label&&(a.type="image",a.url="img/m-o.png");if("pseudoazimuthal"==sel){a.type="image";a.url="img/mby.png";if("Hammer"==a.label||"Winkel 3"==a.label)c=!0,a.type="image",a.url=uselent?"img/mbok.png":"img/bok.png";if("Aitoff"==a.label||"Wagner 7"==
a.label||"Wagner 8"==a.label||"Wagner 9"==a.label)a.type="image",a.url=uselent?"img/mbo.png":"img/b,o.png"}"miscellaneous"==sel&&(a.color="#AAA","Hammer"==a.label||"Winkel 3"==a.label)&&(a.type="image",a.url=uselent?"img/mbok.png":"img/bok.png");"polyhedral"==sel&&(a.type="image",a.url="img/icosa.png");if("equal-area"==sel||"equidistant"==sel)if("Sinusoidal"==a.label||"Werner"==a.label)a.type="image",a.url="img/gpb,g.png";"other same shapes"==sel&&a.attributes[i].startsWith("same shape g")&&(val=
a.attributes[i].substring(12),c=!0,a.type="image",a.url="img/g"+val+".png");if("similar shape"==sel&&("Boggs eumorphic"==a.label||"Siemon IV"==a.label||"Putni\u0146\u0161 P1"==a.label||"Putni\u0146\u0161 P2"==a.label||"Putni\u0146\u0161 P3'"==a.label||"Putni\u0146\u0161 P4'"==a.label||"Wagner 1"==a.label||"Wagner 2"==a.label||"Wagner 3"==a.label)){"Boggs eumorphic"==a.label&&(c=!0,a.type="image",a.url="img/g-r.png");"Siemon IV"==a.label&&(c=!0,a.type="image",a.url="img/g-rk.png");if("Putni\u0146\u0161 P1"==
a.label||"Putni\u0146\u0161 P2"==a.label)c=!0,a.type="image",a.url="img/gray-r.png";if("Putni\u0146\u0161 P3'"==a.label||"Putni\u0146\u0161 P4'"==a.label)c=!0,a.type="image",a.url="img/gray-ok.png";if("Wagner 1"==a.label||"Wagner 2"==a.label||"Wagner 3"==a.label)c=!0,a.type="image",a.url="img/gray-o.png"}"(may be) interrupted"==sel&&("Sinusoidal"==a.label&&(c=!0,a.type="image",a.url="img/gpb,g.png"),"Boggs eumorphic"==a.label&&(c=!0,a.type="image",a.url="img/g-r.png"),"Siemon IV"==a.label&&(c=!0,
a.type="image",a.url="img/g-rk.png"));"Bonne group"==sel&&"Sinusoidal"==a.label&&(c=!0,a.type="image",a.url="img/gpb,g.png");"cylindrical equal-area family"!=sel&&"stretching"!=sel||!a.attributes[i].startsWith("cylindrical equal-area")||(c=!0,a.type="image",a.url="img/c,m.png");if("fusion"==sel&&"fusion-src"==a.attributes[i]||"blending"==sel&&"blending-src"==a.attributes[i]||"construction"==sel&&"const-src"==a.attributes[i]){if("fusion-src"==a.attributes[i]||"blending-src"==a.attributes[i]||"const-src"==
a.attributes[i])c=!0,a.type="def",a.color=colormap[0];if("Hammer"==a.label||"Aitoff"==a.label)a.type="image",a.url="img/r,m.png";"Mollweide"==a.label&&(a.type="image",a.url="img/r,c.png");"Collignon"==a.label&&(a.type="image",a.url="img/c,r.png")}}c||(a.cat1viz=!1);c&&a.cat2viz&&b++;return c&&a.cat2viz&&a.yearviz},"categories").apply();updateCountinfo(b)}
function morecatChange(){$("#comp-div").empty().hide();nodes=s.graph.nodes();edges=s.graph.edges();sel=$(this).val();for(i=0;i<nodes.length;i++)nodes[i].cat2viz=!0;if("== no more filters =="==sel){var c=0;filter.undo("categories").nodesBy(function(b){b.type=b.type2;b.color=b.color2;b.url=b.url2;b.cat1viz&&c++;return b.cat1viz},"categories").apply()}else c=0,filter.undo("categories").nodesBy(function(b){var a=!1;b.type=b.type2;b.color=b.color2;b.url=b.url2;for(i=0;i<Object.values(b.attributes).length;i++)b.attributes[i]==
sel&&(a=!0);a||(b.cat2viz=!1);a&&b.cat1viz&&c++;return a&&b.cat1viz&&b.yearviz},"categories").apply();updateCountinfo(c);s.refresh()}function search(){$("#comp-div").empty().hide();var c=$(this).val();c=new RegExp(c,"i");$(this).val("");forceCleanNode();nodes=s.graph.nodes();var b=!1;for(j=0;j<nodes.length;j++)c.test(nodes[j].label)&&(nodes[j].color="#c00",nodes[j].colorf="#c00",nodes[j].colorg="#c00",b=!0);b?s.refresh():$(this).val("not found!").select()}
$(document).ready(function(){$("#search-pane").css("left",parseInt($("#main-pane").css("width"))+30);$("#search-box").bind("change",search).focus();$("#cat-l1").change(catChange);$("#cat-l2").change(subcatChange);$("#cat-more").change(morecatChange);$("#reset-all").click(catReset);$("#uselent").change(function(){$("#comp-div").empty().hide();uselent=!uselent;$("#cat-more").empty();$("#cat-more").append("<option>== no more filters ==</option>");$("#cat-more").append("<option>cylindrical</option>");
