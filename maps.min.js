...
$(b).show();if(a){nextptr=0;neighbor=c.neighbors;for(j=0;j<edges.length;j++)edges[j].color="#000";visnb=[];visedge=[];c.edges.forEach(function(a,b){0!=b&&(edges[a].color=edges[a].defaultcolor,vis=nodes[c.neighbors[b]].cat1viz&&nodes[c.neighbors[b]].cat2viz&&nodes[c.neighbors[b]].yearviz)&&(visnb.push(c.neighbors[b]),visedge.push(c.edges[b]))});ptrlen=Object.values(visedge).length;0!=ptrlen&&($(b).append('<hr><div id="show-rel" style="text-align:center"><button id="show-btn">show relations</button></div>'),
$(b).append('<div id="rel-area"</div>'),s.refresh(),$("#show-btn").click(function(){1<ptrlen?$("#show-btn").text("next relations"):$("#show-btn").hide();for(j=0;j<edges.length;j++)edges[j].color="#000";nextptr>=ptrlen&&(nextptr=0);edges[visedge[nextptr]].color=edges[visedge[nextptr]].defaultcolor;s.refresh();$("#rel-area").empty();$("#rel-area").append("<p><< see <b>"+nodes[visnb[nextptr]].label+" projection</b> ("+(nextptr+1)+"/"+ptrlen+") in left side panel</p>");1<ptrlen&&$("#rel-area").append('<div style="text-align:center"><button id="prev-btn">prev relations</button></div>');
...
"-pane").stop().fadeOut(3E3)});$(".popup-div").mouseover(function(){$(this).stop().fadeIn();return!1});$(".popup-div").mouseout(function(){$(this).stop().fadeOut(3E3);return!1});var b=$(document).height(),c=$(document).width();c<b?($("#autofade").html('<div style="text-align: center;position: relative;top:10%; font-size: 24px; color: #f4f0e4;">Warning: You may view on <span style="border: solid 1px;color: #f4f0e4;">mobile</span> device, content is not optimized appropriately.</div>'),mobile_dev=!0,
$("#search-box").attr("disabled","disabled"),$("<br>").insertBefore("#uselent")):(1280>c||600>b)&&$("#autofade").html('<div style="text-align: center;position: relative;top:10%; font-size: 24px; color: #f4f0e4;">Warning: Screen resolusion may be too <span style="border: solid 1px;color: #f4f0e4;">low</span> to render contents.</div>');1360!=c&&1366!=c&&1440!=c&&1536!=c&&1600!=c&&1680!=c||768!=b&&800!=b&&864!=b&&900!=b&&1024!=b&&1050!=b?$("#autofade").hide().fadeIn(2E3).delay(3E3).fadeOut(3E3):$("#autofade").hide()});
