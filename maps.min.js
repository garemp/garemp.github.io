...
function(b){$("#type-pane").hide();$("#prop-pane").hide();clean=!0;mobile_dev?$("#search-box").val(""):$("#search-box").focus().val("");offset=0;cur_neig=cur_node=null;$("#mapr-relmap").html("");$("#mapr-rel").html("");$("#mapr-reldiv").hide()}),sigma.parsers.json("maps.json",s,function(){filter=new sigma.plugins.filter(s);var b,a=s.graph.nodes(),c=a.length;for(b=0;b<c;b++)a[b].cat1viz=!0,a[b].cat2viz=!0,a[b].yearviz=!0,"Quartic authalic"==a[b].label&&(a[b].y-=8),"SAND 2000"==a[b].label&&(a[b].y-=
8),"Hobo-Dyer"==a[b].label&&(a[b].y-=8),"HEALPix"==a[b].label&&(a[b].y-=5),"Strebe 1995"==a[b].label&&(a[b].y+=8),"Azimuthal equidistant"==a[b].label&&(a[b].x+=5,a[b].y+=5),"Wagner 9"==a[b].label&&(a[b].x-=18),"Hammer"==a[b].label&&(a[b].dy+=3),"Aitoff"==a[b].label&&(a[b].dy-=5),"Lambert cylindrical"==a[b].label&&(a[b].dy-=5);for(b=0;b<c;b++)a[b].size=7,a[b].color=a[b].center?"#333":"#AAA",a[b].colorf="#AAA",a[b].colorg="#AAA";c=s.graph.edges();var e=c.length;for(b=0;b<e;b++)c[b].type="curve",c[b].defaultcolor=
c[b].color;s.refresh();for(b=0;b<e;b++)"curved"==c[b].attributes[0]&&(c[b].type="curvedArrow");s.refresh();updateCountinfo(a.length);nolegendfornodes()}))})});var type_append=!1,prop_append=!1;offset=0;cur_neig=cur_node=null;var clean=!0,nextptr=1,ptrlen=0,neighbor;
...
1<ptrlen&&$("#rel-area").append('<div style="text-align:center"><button id="prev-btn">prev relations</button></div>');var a=b.img,c=nodes[visnb[nextptr]].img;var d=b.name?b.name:b.label+" projection";var e=nodes[visnb[nextptr]].name?nodes[visnb[nextptr]].name:nodes[visnb[nextptr]].label+" projection";d+=" (click image to toggle projections)";e+=" (click image to toggle projections)";$("#comp-div").empty().hide();$("#comp-btn").click(function(){$("#comp-div").append("<span id='comp-name'>"+d+"</span><img id='comp-close' width='20px' style='float: right;' src='img/cross.svg'><br><img id='comp-img' width='600px' src="+
a+"></img>");$("#comp-div").show().css({top:"100px"});$("#comp-img").click(function(){$(this).attr("src")==a?($("#comp-name").text(e),$(this).attr("src",c)):($("#comp-name").text(d),$(this).attr("src",a))});$("#comp-close").click(function(){$("#comp-div").empty().hide()})});$("#prev-btn").click(function(){nextptr--;nextptr--;0>nextptr&&(nextptr=ptrlen-1);$("#show-btn").click();$("#comp-div").empty().hide()});updatePane("#minor-pane",nodes[visnb[nextptr]],!1);$("#minor-pane").append("<hr>");$("#minor-pane").append("<p>"+
edges[visedge[nextptr]].desc+"</p>");nextptr++}))}}function forceCleanNode(c){nodes=s.graph.nodes();for(j=0;j<nodes.length;j++)nodes[j].color="#AAA";s.refresh()}function nolegendfornodes(){$("#colorinfo-pane").empty();mobile_dev&&$("#color-count-info-pane").css({bottom:parseInt($("#drawcvco").css("height"))+20+"px"});$("#colorinfo-pane").append("<div id='legend-title'><b>No Legend for Nodes</b></div>")}var sub_categories=new Map;sub_categories.set("developable surfaces","cylindrical conic azimuthal pseudocylindrical pseudoconic polyconic lenticular miscellaneous".split(" "));
